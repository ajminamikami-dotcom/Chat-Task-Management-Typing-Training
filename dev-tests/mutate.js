...
const LOGIC_MUTANTS = [
  ...
  ["L67", "CSV の受信順並べ替えを外す", "return chats.slice().sort((a, b) => a.id - b.id).map((chat) => csvRow(chat, session));", "return chats.slice().map((chat) => csvRow(chat, session));"],
  ["L68", "CSV を createdAt で並べる（停止の補正でずれる）", "return chats.slice().sort((a, b) => a.id - b.id).map((chat) => csvRow(chat, session));", "return chats.slice().sort((a, b) => a.createdAt - b.createdAt || a.id - b.id).map((chat) => csvRow(chat, session));"],

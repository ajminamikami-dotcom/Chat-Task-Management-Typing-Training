...
const LOGIC_MUTANTS = [
  ...
  ["L24", "返信不要の見落としがあっても称賛", "&& !(noReplyAccuracy !== null && noReplyAccuracy < GOOD)) strengths.push", "&& true) strengths.push"],
  ...
  ["L27", "返信不要の助言を二重に出す", "} else if (noReplyAccuracy !== null && noReplyAccuracy < GOOD) {", "} if (noReplyAccuracy !== null && noReplyAccuracy < GOOD) {"],
  ...
  ["L35", "Level 1 の訂正文の条件を緩める", "else if (level === 1 && chat.correctOpt !== null) replyText", "else if (level === 1) replyText"],
  ...
  ["L46", "Level 1 で返信不要に返信しても正解", "isReplyCorrect = chat.requiresReply === true && result.optionIndex === chat.correctOpt;", "isReplyCorrect = result.optionIndex === chat.correctOpt;"],
